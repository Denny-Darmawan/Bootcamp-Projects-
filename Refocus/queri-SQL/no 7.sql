...
count(consecutive_number)/365 as rata_rata_kecelakaan_perhari,
sum (number_of_fatalities)/365 as rata_rata_kematian_perhari, 
sum(number_of_fatalities)*1.0/count (consecutive_number) as tingkat_kematian_rata_rata_per_kecelakaan
from Data_cleaning 
group by manner_of_collision_name
having sum (number_of_fatalities)/365 > 1
order by rata_rata_kecelakaan_perhari desc