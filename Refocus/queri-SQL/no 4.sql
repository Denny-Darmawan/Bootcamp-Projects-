...
-- filter driver yang mabuk dan yang tidak mabuk --
drunk_drivers as
(select count (case when number_of_drunk_drivers > 0 then 1 end) as drunk_vs_none_drunk, 
count (case when number_of_drunk_drivers = 0 then 1 end) as none_drunk_drivers,
count (consecutive_number) as total_crash 
from Data_cleaning)

-- parent query --
-- Persentase kecelakaan yang disebabkan oleh pengemudi yang mabuk --
select 'drunk_drivers' as drunk_and_none_drunk, drunk_vs_none_drunk, total_crash, 
cast(drunk_vs_none_drunk as numeric) / cast (total_crash as numeric) * 100 as percentage
from drunk_drivers
union
select 'none_drunk_drivers' as drunk_and_none_drunk, none_drunk_drivers, total_crash, 
cast(none_drunk_drivers as numeric) / cast (total_crash as numeric) * 100 as percentage
from drunk_drivers
