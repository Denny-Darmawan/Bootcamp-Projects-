...
-- extrak hari --
Accidents_per_hour as (
  select
    to_char(new_timestamp, 'Day') as day_of_week,
    count(consecutive_number) as sum_of_crash,
	to_char(new_timestamp, 'D') as number_day
  from
    Data_cleaning
  group by day_of_week, number_day
)
